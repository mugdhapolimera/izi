...
#kwargs =  logOHsun, intergridfile, logzlimits, logqlimits ,logzprior, logqprior (logz/q prior have no application in the original code)
    ...
    if ('grid' in kwargs.keys()):    #Interpolate flag is set to 1 and grid is passed to izi
        grid, ngrid, zarr, qarr, dlogz, dlogq = kwargs['grid'], kwargs['ngrid'], kwargs['zarr'], kwargs['qarr'], kwargs['dlogz'], kwargs['dlogq'] 
    else:
        grid  = grid0
        ngrid = len(grid['LOGZ'])
